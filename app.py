...
def init_db():
    ...
    # Bảng Đơn hàng (Orders) - Bổ sung cột payment_method (Phương thức thanh toán)
    cursor.execute('''
    ...
    if cursor.fetchone()[0] == 0:
        ...
        orders_data = [
            ('Lê Hoàng Hải', 1, 1, 15000000, 'Chuyển khoản'),
            ('Trần Ngô Thái Nguyên', 3, 2, 2400000, 'Tiền mặt'),
            ('Quách Văn Trường', 2, 1, 22000000, 'Chuyển khoản'),
            ('Lê Đức Thiện', 4, 1, 800000, 'Tiền mặt'),
            ('Hồ Ngọc Thành', 5, 2, 7000000, 'Chuyển khoản')
        ]
        cursor.executemany("INSERT INTO orders (customer_name, product_id, quantity, total_price, payment_method) VALUES (?, ?, ?, ?, ?)", orders_data)
...
def view_orders():
    conn = sqlite3.connect("store_standalone.db")
    # Lấy thêm cột payment_method ra màn hình
    query = """
        SELECT orders.id, orders.customer_name, products.name AS product_name, 
               orders.quantity, orders.total_price, orders.payment_method 
        FROM orders 
...
# HÀM MUA HÀNG TÍCH HỢP PHƯƠNG THỨC THANH TOÁN
def buy_product(customer_name, product_name, quantity, payment_method):
    ...
    # 1. Cập nhật tồn kho (SQL UPDATE)
    new_stock = p_stock - quantity
    cursor.execute("UPDATE products SET stock = ? WHERE id = ?", (new_stock, p_id))
    
    # 2. Lưu thông tin đơn hàng kèm phương thức thanh toán (SQL INSERT)
    cursor.execute("INSERT INTO orders (customer_name, product_id, quantity, total_price, payment_method) VALUES (?, ?, ?, ?, ?)",
                   (customer_name, p_id, quantity, total_price, payment_method))
    
    conn.commit()
    conn.close()
    
    return f"🎉 Đặt hàng thành công! [{customer_name}] mua [{quantity}x {product_name}]. Tổng: {int(total_price):,} VNĐ. Hình thức: {payment_method}.", view_products(), get_gallery_data(), view_orders()

# =====================================================================
# 3. CHỨC NĂNG AI PHÂN LOẠI KHÁCH HÀNG VIP
# =====================================================================
...
# =====================================================================
# 4. THIẾT KẾ GIAO DIỆN CHỢ MUA BÁN SHOPEE CAO CẤP
# =====================================================================
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    ...
    with gr.Tabs():
        ...
        with gr.TabItem("🍊 Chợ Shopee Mua Hàng"):
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    # Lấy danh sách tên sản phẩm đưa vào Dropdown lựa chọn
                    ...
                    # --- THÊM PHƯƠNG THỨC THANH TOÁN (RADIO BUTTONS) ---
                    buy_payment = gr.Radio(choices=["Tiền mặt", "Chuyển khoản"], value="Tiền mặt", label="💳 Phương thức thanh toán")
        ...
        # TAB 2: QUẢN LÝ KHO HÀNG (DÀNH CHO CHỦ SHOP)
        ...
        # TAB 3: DANH SÁCH ĐƠN HÀNG ĐÃ ĐẶT (HIỂN THỊ CẢ THANH TOÁN)
        with gr.TabItem("📋 Danh sách Đơn hàng (Orders)"):
            ...
            # Đăng ký nút mua hàng gửi thêm input payment
            btn_buy.click(buy_product, inputs=[buy_user, buy_prod_dropdown, buy_qty, buy_payment], outputs=[shopee_msg, table_products, product_gallery, table_orders])
